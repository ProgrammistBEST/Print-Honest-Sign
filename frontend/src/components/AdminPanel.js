...
const AdminPanel = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: "Модели успешно добавлены",
        type: "success",
      });
    } catch (error) {
      ...
      setSnackbar({
        open: true,
        message: "Ошибка добавления моделей",
        type: "error",
      });
    }
  };
  ...
  socket.on(
    "upload_status",
    ({ progress, message, placePrint, arrayAddingModels }) => {
  ...
  const filteredDataWaiting = Object.entries(InfoAboutHonestSing).reduce(
    (acc, [brand, data]) => {
      const filteredModelsWaiting = data.models.filter(
        (model) =>
          (!modelFilterWaiting ||
            model.model
              .toLowerCase()
  ...
  return (
    <div className="admin-wrapper">
      <button
        className="OpenPanelAmin open-panel-admin btn-submit"
        onClick={OpenWindowAdmin}
      >
        Панель администратора
      </button>
      <div
        className={`admin-container ${windowAdmin ? "panelOn" : "panelOff"}`}
      >
        <section
          className={CheckStatus ? "Admin" : "NonAdmin"}
          style={{
            width: "400px",
            padding: "20px",
        ...
        >
          ...
          {placePrint && (
            ...
            >
              Вы выбрали: {placePrint}
            </p>
          )}

          <label
            htmlFor="printerForBarcode"
            style={{
              fontWeight: "bold",
              marginBottom: "5px",
              display: "block",
          ...
          {printerForBarcode && (
            ...
            >
              Вы выбрали: {printerForBarcode}
            </p>
          )}
          <label
            htmlFor="printerForHonestSign"
            style={{
              fontWeight: "bold",
              marginBottom: "5px",
              display: "block",
            ...
            >
              ...
              <tbody>
                {filteredItems.map((item, index) => (
                  <tr key={index} id={`row-${index}`} className="fade-out-row">
        ...
        <section className="admin-panel">
          ...
          <div
            className={`${CheckStatus ? "Admin" : "NonAdmin"} admin-functions`}
          >
              ...
              >
                <article className="add-section">
                  ...
                  >
                    ...
                    <span>
                      {addMultiModel
                        ? "Добавятся как мультимодели"
                        : "Добавятся помодельно"}
                    </span>
                  </div>
