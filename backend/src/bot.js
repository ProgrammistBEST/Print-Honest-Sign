...
const fetchApiToken = async (id, company_name, category) => {
    try {
        const response = await fetch(
            `http://localhost:6501/getApiToken?id=${id}&company_name=${company_name}&category=${category}`
        );
...
// Пример вызова
fetchApiToken(876, 'Armbest', 'OZON').then((token) => {
    if (token) {
...
let token = fetchApiToken(23, 'Armbest', 'OZON');
