...
const { getCategoryByModel } = require("../controllers/models.js");
...
const compareFiles = async (req, res) => {
  ...
  if (!tableName || !normalizedBrand) {
    return res
      .status(400)
      .json({ error: "Некорректные данные о бренде или месте" });
  }
  ...
  try {
    ...
    const dataDB = await getDataFromDB(brand);
...
async function getDataFromDB(brand) {
...
function parseExcelDataToObjects(data) {
  ...
  for (let i = 4; i < data.length; i++) {
    ...
    if (!vendorCode) {
      ...
      const sizes = (currentSizes.length > 0 ? currentSizes : defaultSizes)
        .map((size, index) => {
